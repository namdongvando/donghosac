...
class Controller_mproduct extends Controller_backend
{
    ...
    function index()
    {
        ...
        $PagesIndex = $_GET["PagesIndex"] ?? 1;
        $PagesNumber = $_GET["PagesSize"] ?? 10;
        $options["name"] =  $_GET["name"] ?? "";
        $options["catID"] =  $_GET["catID"] ?? "";
        $a = new \Model\Category();
        $p = new \Model\Products();
        $dataResult = [];
        $sum = 0;
        $b = $p->ProductsNameAllPTNangCao($options, $PagesIndex, $PagesNumber, $sum);
        $dataResult["params"]["PagesIndex"] = $PagesIndex;
        $dataResult["items"] = $b;
        $dataResult["params"]["TotalPages"] = ceil($sum / $PagesNumber);
        $dataResult["params"]["total"] = $sum;
        $dataResult["params"]["PagesSize"] = $PagesNumber;
        $dataResult["params"]["PagesIndex"] = $PagesIndex;
        $dataResult["params"]["catID"] = $options["catID"];
        $dataResult["params"]["name"] = $options["name"];
        $this->ViewTheme($dataResult, Model_ViewTheme::get_viewthene(), "mproduct");
    }
    ...
    function editproduct()
    {
        ...
        if (isset($_POST["LuuSanPham"])) {
            ...
            $editP = $this->Product->ProductsByID($SanPhamPost['ID'], FALSE);
            ...
            $editP['Summary'] = input::test_input($SanPhamPost["Summary"]);
            $editP['Content'] = input::test_input($SanPhamPost["Content"]);
            ...
            $editP['Title'] = $SanPhamPost["Title"];
            $editP['Description'] = $SanPhamPost["Description"];
            $editP['Keyword'] = $SanPhamPost["Keyword"];
            $editP['DateCreate'] = date("Y-m-d H:i:s", time());
    ...
    function addproduct()
    {
        if (isset($_POST["ThemSanPham"])) {
            ...
            if (!$editP) {
                ...
                $editP['Description'] = $SanPhamPost["Description"];
                $editP['BuyTimes'] = 0;
                $editP['isShow'] = isset($SanPhamPost["isShow"]) ? 1 : 0;
                $editP['lang'] = "vi";
                $this->Product->AddProducts($editP);
                $this->Product->_header("/mproduct/editproduct/" . $editP["ID"]);
            }
        }
        $this->ViewTheme("", Model_ViewTheme::get_viewthene(), "mproduct");
    }
